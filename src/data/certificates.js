let certificatesArr = [
    {
        'name': 'CS50x Harvard',
        'image': 'https://github.com/katayoon-rb/Portfolio/blob/main/src/images/CS50x.png?raw=true',
        'link': 'https://cs50.harvard.edu/certificates/8c1603e7-97df-4595-9f4e-5c49c6184676'
    },
    {
        'name': 'CS50p Harvard',
        'image': 'https://github.com/katayoon-rb/Portfolio/blob/main/src/images/CS50p.png?raw=true',
        'link': 'https://certificates.cs50.io/6567aba5-9eda-480e-a000-36f9a5bed67a'
    },
    {
        'name': '50 Projects In 50 Days',
        'image': 'https://github.com/katayoon-rb/Portfolio/blob/main/src/images/50%20Projects%20In%2050%20Days.jpg?raw=true',
        'link': 'https://www.udemy.com/certificate/UC-d5a283ed-9a03-4048-9bf2-f972c04d9c5b/'
    },
    {
        'name': 'NodeJs Udemy',
        'image': 'https://github.com/katayoon-rb/Portfolio/blob/main/src/images/NodeJs.jpg?raw=true',
        'link': 'https://www.udemy.com/certificate/UC-2caf8b66-0358-42f8-bc0b-e5a1c176a308/'
    },
    {
        'name': 'Basic Javascript - HackerRank',
        'image': 'https://github.com/katayoon-rb/Portfolio/blob/main/src/images/Javascript_Basic.jpg?raw=true',
        'link': 'https://www.hackerrank.com/certificates/a8c30f68a3d5'
    },
    {
        'name': 'Advanced Javascript - HackerRank',
        'image': 'https://github.com/katayoon-rb/Portfolio/blob/main/src/images/Javascript_Intermediate.jpg?raw=true',
        'link': 'https://www.hackerrank.com/certificates/a4802c8fe974'
    },
    {
        'name': 'Basic SQL - HackerRank',
        'image': 'https://github.com/katayoon-rb/Portfolio/blob/main/src/images/SQL_Basic.jpg?raw=true',
        'link': 'https://www.hackerrank.com/certificates/c46a36606303'
    },
    {
        'name': 'Intermediate SQL - HackerRank',
        'image': 'https://github.com/katayoon-rb/Portfolio/blob/main/src/images/SQL_Intermediate.jpg?raw=true',
        'link': 'https://www.hackerrank.com/certificates/085520850738'
    },
    {
        'name': 'Python - HackerRank',
        'image': 'https://github.com/katayoon-rb/Portfolio/blob/main/src/images/Python_Basic.jpg?raw=true',
        'link': 'https://www.hackerrank.com/certificates/cae6dc56381c'
    },
    {
        'name': 'Java - HackerRank',
        'image': 'https://github.com/katayoon-rb/Portfolio/blob/main/src/images/Java_Basic.jpg?raw=true',
        'link': 'https://www.hackerrank.com/certificates/1f2c78548370'
    },
    {
        'name': 'React - HackerRank',
        'image': 'https://github.com/katayoon-rb/Portfolio/blob/main/src/images/React_Basic.jpg?raw=true',
        'link': 'https://www.hackerrank.com/certificates/7a286a5a62cf'
    },
    {
        'name': 'Frontend Developer (React) - HackerRank',
        'image': 'https://github.com/katayoon-rb/Portfolio/blob/main/src/images/Frontend.jpg?raw=true',
        'link': 'https://www.hackerrank.com/certificates/dedb864c29fe'
    },
    {
        'name': 'Intro to SQL - Kaggle',
        'image': 'https://github.com/katayoon-rb/Portfolio/blob/main/src/images/Intro_SQL.png?raw=true',
        'link': ''
    },
    {
        'name': 'Advanced SQL - Kaggle',
        'image': 'https://github.com/katayoon-rb/Portfolio/blob/main/src/images/Advanced_SQL.png?raw=true',
        'link': ''
    },
    {
        'name': 'Python - Kaggle',
        'image': 'https://github.com/katayoon-rb/Portfolio/blob/main/src/images/Python.png?raw=true',
        'link': ''
    },
    {
        'name': 'Machine Learning - Kaggle',
        'image': 'https://github.com/katayoon-rb/Portfolio/blob/main/src/images/Machine_Learning.png?raw=true',
        'link': ''
    },
    {
        'name': 'Pandas - Kaggle',
        'image': 'https://github.com/katayoon-rb/Portfolio/blob/main/src/images/Pandas.png?raw=true',
        'link': ''
    },
    {
        'name': 'Intermediate Machine Learning - Kaggle',
        'image': 'https://github.com/katayoon-rb/Portfolio/blob/main/src/images/Intermediate_Machine_Learning.png?raw=true',
        'link': ''
    },
    {
        'name': 'Deep Learning - Kaggle',
        'image': 'https://github.com/katayoon-rb/Portfolio/blob/main/src/images/Deep_Learning.png?raw=true',
        'link': ''
    },
    {
        'name': 'Computer Vision - Kaggle',
        'image': 'https://github.com/katayoon-rb/Portfolio/blob/main/src/images/Computer_Vision.png?raw=true',
        'link': ''
    },
    {
        'name': 'Machine Learning Explainability - Kaggle',
        'image': 'https://github.com/katayoon-rb/Portfolio/blob/main/src/images/Machine_Learning_Explainability.png?raw=true',
        'link': ''
    },
]

export default certificatesArr
